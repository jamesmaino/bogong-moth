...
# TO DO
# Garry to send through updated sig catch parameters
...
NSTEPS <- 1 # number of steps to run
...
# Case sensitive!
LOCATIONS <- c("FOWLERS GAP")

DATES <- c() # leave empty to run all sig_catch dates for selected season/year. Dates overide season, year
MONTHS <- c(1:12) # leave empty to run all sig_catch dates for selected season/year. Months overide season
...
warning("look at time conversion and time ")
sims <- list()
for (i in 1:nrow(d)) {
    # for (i in 1) {
    d_i <- d[i, ]
        ...
        for (j in 1:NSTEPS) {
            ...
            trajectory1 <-
                hysplit_trajectory(
                    ...
                    duration = DURATION,
                    days = d_i$date - 1,
                    daily_hours = 24 + START_TIME + AEDT_TO_UTC_OFFSET,
                    direction = "backward",
                    met_type = "reanalysis",
                    extended_met = TRUE,
            ...
            if (trajectory3$air_temp - 273.15 < 10) {
...
# d %>%
#     group_by(lat, lon, date = as.Date(date)) %>%
#     tally()

# sims %>%
#     group_by(lat, lon, date = as.Date(traj_dt)) %>%
#     tally()
